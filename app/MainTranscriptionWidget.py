...
# --- Processing Logic ---
    ...
    def start_transcription(self):
        ...
        if not self.current_recording_data:
            show_error_message(self, 'No Recording Selected',
    ...
    def start_gpt4_processing(self):
        ...
        if not self.current_recording_data.get('raw_transcript'):
            show_error_message(self, 'No Transcript',
    ...
    def on_smart_format_completed(self, formatted_html):
        ...
        if formatted_html:
            ...
            show_error_message(self, 'Formatting Failed',
    ...
    def on_recording_item_selected(self, recording_item):
        ...
        def on_recording_loaded(recording: Recording):
            if recording:
                ...
                show_error_message(
                    self, 'Error', f"Could not load recording data for ID: {recording_id}")
                self.current_recording_data = None
                self.transcript_text.clear()
                self.update_ui_state()
    ...
    def update_ui_state(self):
        ...
        self.transcript_text._toolbar_actions['process_with_gpt4'].setEnabled(
            has_raw_transcript and not self.is_transcribing and not self.is_processing_gpt4)
        # Can smart format if text editor has content and not busy
        can_smart_format = bool(self.transcript_text.toPlainText().strip()
                                ) and not self.is_transcribing and not self.is_processing_gpt4
