...
with patch('app.controllers.transcription_controller.TranscriptionThread') as MockTranscriptionThread, \
     patch('app.controllers.transcription_controller.ThreadManager') as MockThreadManager, \
     patch('app.controllers.transcription_controller.get_api_key', return_value='fake-api-key'), \
     patch('app.controllers.transcription_controller.os.path.exists', return_value=True), \
     patch('app.controllers.transcription_controller.os.path.getsize', return_value=1024*1024): # 1MB
...
class TestTranscriptionController(unittest.TestCase):
    ...
    def test_input_validation(self):
        ...
        with patch('app.controllers.transcription_controller.os.path.exists', return_value=False):
        ...
        with patch('app.controllers.transcription_controller.os.path.getsize', return_value=350*1024*1024):  # 350MB
            result = self.controller._validate_inputs(self.recording)
            self.assertFalse(result)
    ...
    def test_on_transcription_completed(self):
        ...
        self.db_manager.update_recording.assert_called_once_with(
            self.recording.id, ANY, raw_transcript=transcript, raw_transcript_formatted=None
        )
        
        # Test with formatted transcript (speaker labels)
        formatted_transcript = "SPEAKER_0: This is speaker 0.\nSPEAKER_1: This is speaker 1."
        self.db_manager.update_recording.reset_mock()
        
        self.controller._on_transcription_completed(self.recording, formatted_transcript)
        
        # Verify database update was called with formatting
        self.db_manager.update_recording.assert_called_once_with(
            self.recording.id, ANY, 
    ...
    def test_cancel(self):
        ...
        self.controller.status_update.emit.assert_called_with("Canceling transcription...")
