...
class TestGPTController(unittest.TestCase):
    ...
    def test_process_validation(self):
        ...
        result = self.controller.process(
            recording_no_transcript, self.prompt, self.config, self.busy_guard_callback
        )
        ...
        with patch("app.controllers.gpt_controller.get_api_key", return_value=None):
            result = self.controller.process(
                self.recording, self.prompt, self.config, self.busy_guard_callback
            )
    ...
    def test_smart_format(self):
        ...
        result = self.controller.smart_format(
            text_to_format, self.config, self.busy_guard_callback, completion_callback
        )
        ...
        self.assertIn(
            "Format the following text using HTML", call_args["prompt_instructions"]
        )
        self.assertEqual(
    ...
    def test_cancel(self):
        ...
        self.controller.threads = {
            "test_thread": {"thread": mock_thread, "busy_guard": MagicMock()}
        }
