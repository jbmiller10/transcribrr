                ...
                try:
                    ...
                    if not op_type:
                        ...
                        self.error_occurred.emit(
                            "invalid_operation", "Invalid operation format: missing type")
                        continue
                    ...
                    try:
                        ...
                        try:
                            ...
                            self._log_error("Database connection issue",
                                            health_error, level="warning")
                            # Try to reconnect
                            try:
                                self.conn = get_connection()
                                self.conn.execute("PRAGMA foreign_keys = ON;")
                        ...
                        if op_type == 'execute_query':
                            ...
                            is_modifying_query = any(query_lower.startswith(prefix)
                            ...
                            if is_modifying_query:
                                try:
                                    ...
                                    self._log_error("SQL error", sql_error,
                                                    "modifying query", emit_signal=False)
                                    raise RuntimeError(
                            ...
                            try:
                                ...
                                self._log_error("Error creating table", table_error,
                                                op_type, emit_signal=False)
                                raise RuntimeError(
                            ...
                            try:
                                ...
                                self._log_error("Duplicate path", dupe_error,
                                                op_type, level="warning")
                                raise  # Re-raise for special handling in the exception block
                            except Exception as create_error:
                                self._log_error("Error creating recording",
                                                create_error, op_type, emit_signal=False)
                            ...
                            try:
                                ...
                                self._log_error("Error getting recordings",
                                                get_error, op_type, emit_signal=False)
                            ...
                            try:
                                ...
                                self._log_error(
                                    f"Error getting recording by ID {op_args[0]}", get_error, op_type, emit_signal=False)
                            ...
                            try:
                                ...
                                self._log_error(
                                    f"Error updating recording {op_args[0]}", update_error, op_type, emit_signal=False)
                            ...
                            try:
                                ...
                                self._log_error(
                                    f"Error deleting recording {op_args[0]}", delete_error, op_type, emit_signal=False)
                            ...
                            try:
                                ...
                                self._log_error("Error searching recordings",
                                                search_error, op_type, emit_signal=False)
...
class DatabaseManager(QObject):
    ...
    def execute_query(self, query, params=None, callback=None, return_last_row_id=False, operation_id=None):
        ...
        self.worker.add_operation('execute_query', operation_id, query,
                                  params or [], return_last_row_id=return_last_row_id)
